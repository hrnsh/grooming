...
<%@page import="kr.co.gudi.reserve.controller.ReserveController"%>
...
//	1일의 요일을 계산한다(자주 쓰이기 때문에 변수로 선언해두기)
		int first = ReserveController.weekDay(year, month, 1);
	
//	1일이 출력될 위치 전에 전달의 마지막 날짜들을 넣어주기위해 전 달날짜의 시작일을 계산한다.
		int start = 0 ;
		start = month ==1? ReserveController.lastDay(year-1, 12)- first : ReserveController.lastDay(year, month-1)- first;
...
/* 1일부터 달력을 출력한 달의 마지막 날짜까지 반복하며 날짜를 출력 */
		for(int i = 1; i <= ReserveController.lastDay(year, month); i++){
			/* 요일별로 색깔 다르게 해주기위해 td에 class 태그걸어주기 */
			switch(ReserveController.weekDay(year, month, i)){
				case 0 :
...
/* 출력한 날짜(i)가 토요일이고 그달의 마지막 날짜이면 줄을 바꿔주기 */
			if(ReserveController.weekDay(year, month, i) == 6 && i != ReserveController.lastDay(year, month)){
				out.println("</tr><tr>");			
			}
		}
		if(ReserveController.weekDay(year, month, ReserveController.lastDay(year, month)) !=6){
			for(int i = ReserveController.weekDay(year, month, ReserveController.lastDay(year, month))+1; i < 7; i++){
				out.println("<td></td>");	
			}
		}
